...
module Make(Elt: ELT) = struct
  ...
  let rem x = 
	...
	| Empty -> Empty
	| Singleton y -> 
	    if Elt.equal x y then Empty else s
	| Add(y, s') -> 
	    if Elt.equal x y then s' else 
	      let s'' = remx s' in
 ...
 let disjoint s1 s2 = 
   let notmem s x = not (mem x s) in
     for_all (notmem s2) s1 &&
     for_all (notmem s1) s2
 let union s1 s2 = 
  ...
  let pp fmt s = 
    ...
    let rec printl = function
      | [] -> ()
      | [x] -> Elt.pp fmt x
      | x :: xl -> Elt.pp fmt x; Format.fprintf fmt ", "; printl xl
    in
      printl (to_list s);
      Format.fprintf fmt "}@]@?"
end 
