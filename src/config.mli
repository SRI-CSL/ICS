...
module Set(S: Sets.S): sig
  ...
  val unchanged : unit -> bool
    (** [unchanged()] holds iff the current configuration has been
      unchanged since the latest [initialize]. *)
...
(** Inference system for manipulating a onfiguration consisting of a finite map. *)
module Map(M: Maps.S): sig
  ...
  val find : M.key -> M.value
    (** [find x] returns [t] if there is a binding [x |-> t] in the
      current configuration; otherwise [Not_found] is raised. *)
  ...
  val set : M.key -> M.value -> unit
    (** [set x t] updates the current configuration with the binding [x |-> t]. *)
...
module Powermap(M: Powermaps.S): sig
  ...
  val is_singleton : M.key -> bool
    (** [is_singleton x]  holds iff the current configuration has only
      one binding of the form [x |-> s]. *)

  val update : M.key -> M.Values.t -> unit
    (** [update x t] updates the current configuration with the 
      binding [x |-> t]. *)
  ...
  val merge : M.key -> M.key -> unit
    (** [merge x y] updates the current configuation with 
      the binding [y |-> find(x) U find(y)] and removes the binding for [x]. *)

  val add : M.key -> M.key -> unit
    (** [add x y] updates the current configuartion with the 
      binding [y |-> {x} U find(y)]. *)

  val rem : M.key -> M.key -> unit
    (** [rem x y] updates the current configuration with
      the binding [y |-> find(y)\{x}]. *)
end


(** Inference system for manipulating a onfiguration consisting of a substitution. *)
module Subst(S: Subst.S): sig
  type t = S.t
      (** Configurations are substitions. *)

  val initialize : S.t -> unit
    (** [initialize s] initializes the {i current configuration} with
      the substitution [s]. *)
  ...
  val reset : unit -> unit
    (** [reset()] resets the current configuration to the empty
      substitution. *)
  ...
  val lookup : S.var -> S.trm
    (** [lookup x] returns [t] if there is a binding [x |-> t]
      in the current configuration; otherwise, [Not_found] is raised. *)

  val mem : S.var -> bool
    (** [mem x] holds iff there is a binding [x |-> t] in the
      current configuration. *)

  val apply : S.trm -> S.trm
    (** [apply t] replaces all [x] in [t] with [s] for bindings [x |-> s]
      in the current configuration. *)

  val add : S.var -> S.trm -> unit
    (** For, [x] not in the domain of the current configuration,
      [add x t] adds the binding [x |-> t] to the current configuration
      and replaces all occurrences of [x] in codomain terms with [s]. *)
  ...
  val compose : S.t -> unit
    (** If [rho] is domain-disjoint from the current configuration, then
      ...
      and replaces occurrences of domain variables [x] in codomain
      terms with [t], where [x |-> t] is in [rho]. *)

  val update : S.var -> S.trm -> unit
    (** [update x t] replaces a binding [x |-> s] with [x |-> t]. *)

  val remove : S.var -> unit
    (** [remove x] removes a binding [x |-> t] from the current 
      configuation. *)

  val fold : (S.var -> S.trm -> 'a -> 'a) -> 'a -> 'a
    (** For [rho] the current configuration of the form [{x{1} |-> t{1},...,x{n} |-> t{n}}], 
      the iterator [fold f e] returns  [f x{1}  t{1} (... (f x{n} t{n} e)...)];
      the order of accumulation is unspecified. *)
    
  val iter : (S.var -> S.trm -> unit) -> unit
    (**  For [rho] the current configuration, [iter f rho] applies [f x t] for all 
      bindings [x |-> t] of [rho]. The order of application is unspecified. *)
  ...
  val choose : (S.var -> S.trm -> bool) -> S.var * S.trm
    (**  For [rho] the current configuration, , [choose p rho] returns [(x, t)] 
      iff there exists a binding [x |-> t] in [rho] with [p x t]; 
