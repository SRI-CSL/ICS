...
(defun ics-canon (state pvs-expr)
  (let* ((ics-expr (translate-to-ics pvs-expr))
	 (ics-term (ics_can (car state) (car ics-expr))))
...
(defmethod translate-to-ics* ((ex name-expr))
  ...
  (cond ((tc-eq ex *true*)
	...
	(t (ics_mk_var (string (id ex)))))) ;; not quite right
...
(defmethod translate-to-ics* ((ex equation))
  (ics_mk_equal (car (translate-to-ics* (args1 ex)))
		(car (translate-to-ics* (args2 ex)))))

(defmethod translate-to-ics* ((ex disequation))
  (ics_mk_diseq (car (translate-to-ics* (args1 ex)))
		(car (translate-to-ics* (args2 ex)))))

(defmethod translate-to-ics* ((ex branch))
  (ics_mk_ite (car (translate-to-ics* (condition ex)))
...
(defmethod translate-to-ics* ((ex negation))
  (cond ((equation? (args1 ex))
	 (ics_mk_diseq (car (translate-to-ics* (args1 (args1 ex))))
		    (car (translate-to-ics* (args2 (args1 ex))))))
	((disequation? (args1 ex))
	 (ics_emk_qual (car (translate-to-ics* (args1 (args1 ex))))
		    (car (translate-to-ics* (args2 (args1 ex))))))
	(t (call-next-method))))
...
(defmethod translate-to-ics* ((ex implication))
  (ics_mk_imp (car (translate-to-ics* (args1 ex)))
	      (car (translate-to-ics* (args2 ex)))))

(defmethod translate-to-ics* ((ex conjunction))
  (ics_mk_and (car (translate-to-ics* (args1 ex)))
	      (car (translate-to-ics* (args2 ex)))))

(defmethod translate-to-ics* ((ex disjunction))
  (ics_mk_or (car (translate-to-ics* (args1 ex)))
	     (car (translate-to-ics* (args2 ex)))))

(defmethod translate-to-ics* ((ex application))
  (let ((op (operator ex)))
    (cond ((tc-eq op (plus-operator))
	   (ics_mk_plus (ics_cons (car (translate-to-ics* (args1 ex)))
				  (ics_cons (car (translate-to-ics* (args2 ex)))
					    (ics_nil)))))
	  ((tc-eq op (difference-operator))
	   (ics_mk_minus (car (translate-to-ics* (args1 ex)))
			 (car (translate-to-ics* (args2 ex)))))
	  ((tc-eq op (unary-minus-operator))
	   (ics_mk_unary_minus (car (translate-to-ics* (args1 ex)))))
	  ((tc-eq op (times-operator))
	   (ics_mk_times (ics_cons (car (translate-to-ics* (args1 ex)))
				   (ics_cons (car (translate-to-ics* (args2 ex)))
					     (ics_nil)))))
	  ((tc-eq op (divides-operator))
	   (ics_mk_div (car (translate-to-ics* (args1 ex)))
		       (car (translate-to-ics* (args2 ex)))))
	  ((tc-eq op (greatereq-operator))
	   (ics_mk_ge (car (translate-to-ics* (args1 ex)))
		      (car (translate-to-ics* (args2 ex)))))
	  ((tc-eq op (greater-operator))
	   (ics_mk_gt (car (translate-to-ics* (args1 ex)))
		      (car (translate-to-ics* (args2 ex)))))
	  ((tc-eq op (less-operator))
	   (ics_mk_lt (car (translate-to-ics* (args1 ex)))
		      (car (translate-to-ics* (args2 ex)))))
	  ((tc-eq op (lesseq-operator))
	   (ics_mk_le (car (translate-to-ics* (args1 ex)))
		      (car (translate-to-ics* (args2 ex)))))
	  ((tc-eq op (integer_pred))
	   (ics_mk_int (car (translate-to-ics* (args1 ex)))))
	  ((union-set? op)
	  ...
	  ((and (conc-bv? op) (fixed-bv? ex))
	   (let ((n (width-of-fixed-bv (args1 ex)))
	     ...
	     (ics_mk_bv_conc (car (translate-to-ics* n))
			     (car (translate-to-ics* (args1 ex)))
...
(defmethod translate-to-ics* ((ex tuple-expr))
  (ics_mk_tup (car (translate-to-ics* (exprs ex)))))
  ...
  (let ((expr (if (consp expr-pointer)
	...
	(parse-integer
	 ...
	 (make!-disequation (translate-from-ics (ics_fst dexpr))
			    (translate-from-ics (ics_snd dexpr)))))
      ((plusp (ics_is_neg expr))
       (let ((dexpr (ics_d_neg expr)))
	 (make-less (translate-from-ics dexpr) (zero))))
      ((plusp (ics_is_nonpos expr))
       (let ((dexpr (ics_d_nonpos expr)))
	 (make-lesseq (translate-from-ics dexpr) (zero))))
      ((plusp (ics_is_pos expr))
       (let ((dexpr (ics_d_pos expr)))
	 (make-less (zero) (translate-from-ics dexpr))))
      ((plusp (ics_is_nonneg expr))
       (let ((dexpr (ics_d_nonneg expr)))
	 (make-lesseq (zero) (translate-from-ics dexpr))))
      ((plusp (ics_is_int expr))
       (let ((dexpr (ics_d_int expr)))
	 (make-integer_pred (translate-from-ics dexpr))))
      ((plusp (ics_is_real expr))
       (let ((dexpr (ics_d_real expr)))
	 (make-real_pred (translate-from-ics dexpr))))
      ;; other props
      ;; TODO: reconstruct logical connectives from if-then-else props
      ((plusp (ics_is_and expr))
       (let ((dexpr (ics_d_and expr)))
	 (make!-conjunction (translate-from-ics (ics_fst dexpr))
			    (translate-from-ics (ics_snd dexpr)))))
      ((plusp (ics_is_or expr))
       (let ((dexpr (ics_d_or expr)))
	 (make!-disjunction (translate-from-ics (ics_fst dexpr))
			    (translate-from-ics (ics_snd dexpr)))))
      ((plusp (ics_is_neg expr))
       (let ((dexpr (ics_d_neg expr)))
	 (make!-negation (translate-from-ics dexpr))))
      ((plusp (ics_is_imp expr))
       (let ((dexpr (ics_d_imp expr)))
	 (make!-implication (translate-from-ics (ics_fst dexpr))
			    (translate-from-ics (ics_snd dexpr)))))
      ((plusp (ics_is_iff expr))
       (let ((dexpr (ics_d_iff expr)))
	 (make!-iff (translate-from-ics (ics_fst dexpr))
		    (translate-from-ics (ics_snd dexpr)))))
