...
module Make(C: COEFF)(X: INDETERMINATE) = struct
  ...
  let pp fmt p =   
    Format.fprintf fmt "@[";
    (if Map.is_empty p.monomials then
       C.pp fmt p.constant
     else
       let ppMonomial (c, x) =  
	 if C.equal c C.one then X.pp fmt x else
	   (C.pp fmt c; Format.fprintf fmt "*"; X.pp fmt x )
       in
       let ppSignedMonomial (c, x) = 
	 if C.compare c C.zero > 0  then 
	   (Format.fprintf fmt " + "; ppMonomial (c, x))
	 else
	   (Format.fprintf fmt " - "; ppMonomial (C.minus c, x))
       in
       let ma = orderedMonomials p in
       let n = Array.length ma in
       let (c0, x0) = Array.get ma 0 in 
	 if C.equal p.constant C.zero then ppMonomial (c0, x0) else 
	   (C.pp fmt p.constant; ppSignedMonomial (c0, x0));
	 for i = 1 to n - 1 do
	   ppSignedMonomial (Array.get ma i)
	 done);
      Format.fprintf fmt "@]@?"
  ...
  let compare p1 p2 =  
	...
	if h1 > h2 then 1 else
	  if C.equal p1.constant p2.constant && Map.equal p1.monomials p2.monomials then 0 else
  ...
  let iter fc fm p =
    ...
    fc p.constant; Map.iter fm p.monomials
  ...
  let eval rho p =
    assert(well_formed p);
    let fc c = c 
    and fm x c = C.add (C.mult c (Interp.apply rho x)) in
      fold fc fm p
  ...
  (** Checks if the relation [rel [v1;...;vn] v] is not violated for
    a number of randomly generated values for the polynomials [q1;...;qn] and [p].
  ...
  module Check = struct
	...
	let value = eval (fun x -> Map.find x rho) in
      ...
      in
      let rec every i = 
	if i < 0 then true else sat1() && every (i - 1)
      in
	every !probe
    ...
    let add p1 p2 p = 
      let addRel v1 v2 v = C.equal (C.add v1 v2) v in 
      let res = valid2 addRel p1 p2 p in
	if not res then Format.eprintf "\nFailed check: add(%s, %s, %s)@?" (to_string p1) (to_string p2) (to_string p);
	res

    let equiv (p1, q1) (p2, q2) = 
  ...
  exception Found
  let choose =
    let coeff = ref (Obj.magic 0) 
    and indet = ref (Obj.magic 0) in
      fun f p ->
	assert(well_formed p);
	let chooseM x c = 
	  if f x c then (coeff := c; indet := x; raise Found) 
	in
	  try 
	    Map.iter chooseM p.monomials;
	    raise Not_found 
	  with
	      Found -> (!indet, !coeff)
  ...
  let is_monomial p = 
    assert(well_formed p);
    C.equal C.zero p.constant &&
    Map.is_singleton p.monomials
  ...
  let monomial c x = 
    if C.equal C.zero c then zero else
      if C.equal C.one c then indet x else
	make C.zero (Map.singleton x c)
           
  let multc c p =
    assert(well_formed p);
    if C.equal C.zero c then zero else
      if C.equal C.one c then p else
	let mc = C.mult c in
	let q = make (mc p.constant) (Map.map mc p.monomials) in
	  assert(well_formed q);
	  assert(let multRel u v = C.equal (mc u) v in Check.valid1 multRel p q);
	  q
  ...
  let neg p =
    ...
    in
      assert(well_formed q);
      assert(let negRel u v = C.equal (C.minus u) v in Check.valid1 negRel p q);
      q

  let addc c p =
    assert(well_formed p);
    if C.equal C.zero c then p else
      let q = make (C.add c p.constant) p.monomials in
	assert(well_formed q);
	assert(let addcRel u v = C.equal (C.add c u) v in Check.valid1 addcRel p q);
	q
  ...
  let add p1 p2 = 
	...
	let add1 x c = 
	  ...
	  try
	    ...
	    let cd = C.add c d in
	      if C.equal C.zero cd then Map.remove x acc else
		Map.set x cd acc
	  with
	      Not_found -> Map.set x c acc
	in
    ...
    in
    let p = 
      try addc (d_constant p1) p2 with Nonnum -> 
	try addc (d_constant p2) p1 with Nonnum -> 
  ...
  let sub p1 p2 = 
    assert(well_formed p1);
    assert(well_formed p2);
    try  addc (C.minus (d_constant p2)) p1 with Nonnum ->
	...
	try
	  let d = Map.find x m' in
	  let cd = C.sub c d in
	    if C.equal C.zero cd then Map.remove x m' else
	      Map.set x cd m'
	with
	    Not_found -> Map.set x (C.minus c) m'
      in
	Map.iter subtract p2.monomials;
	let p = make c' m' in
	  assert(well_formed p);
	  assert(let subRel v1 v2 v = C.equal (C.sub v1 v2) v in 
		   Check.valid2 subRel p1 p2 p);
	  p
  ...
  let minus p = 
    let q = mapc C.minus p in
      assert(let minRel u v = C.equal (C.minus u) v in
	       Check.valid1 minRel p q);
      q
...
(*
    ...
    let ml = p.monomials and nl = q.monomials in
      C.equal (C.minus c) d &&
      failwith "negated: to do"
*)
      
  let map f p = 
    let c = p.constant and ms = p.monomials in
      if Map.is_empty ms then p else
	let accumulate x d = try add (multc d (f x)) with _ -> add (multc d (indet x)) in
	  Map.fold accumulate ms (constant c)

  let replace x p q = 
    let lookup y = 
      if X.equal x y then p else raise Not_found 
    in
      map lookup q

  let rename x y p =  
    assert(well_formed p);
    if X.equal x y then p else
      let q = 
	try
  ...
  let instantiate x c p = 
    let k = coeff x p in
      if C.equal C.zero k then p else
  ...
  let subst p x q = 
    ...
    assert(well_formed q);
    assert(not(mem x q));
    try rename x (d_indet q) p with Nonindet -> 
      try
	...
	let addm z e =
	  try
	    ...
	    let g = C.add f (C.mult d e) in
	      if C.equal C.zero g then Map.remove z m' else
		Map.set z g m'
	  with
	      Not_found ->
		let de = C.mult d e in
		  if C.equal C.zero de then () else
		    Map.set z de m'
	in
	  Map.remove x m';
  ...
  let pivot x y p =
    ...
    let c = p.constant and m = p.monomials in
      try            (* [x = c + d*y + m'] ==> [y =  1/d*x + (-c/d - 1/d*m')] *)
	...
	let m' = Map.empty() in
	let add z e = 
	  if X.equal z y then () else
	    Map.set z (C.mult e neginvd) m'
	in
	  Map.iter add m;
	  let q = addm invd x (make c' m') in
	    ...
	    q
      with
	  Not_found -> raise(Invalid_argument "Polynomial.pivot")

end

(*
module Lazy(Poly: P) (* : (P with type coeff = Poly.C.t and type indet = Poly.X.t) *) = struct
...
module Test = struct
  ...
  module Ops = struct
    ...
    let addm () = 
      ...
      let p = Random.poly() in
	Format.eprintf "\naddm <-- %s %s %s@?" (C.to_string c) (to_string (P.indet x)) (to_string p);
   ...
   let sub () =
	...
	try 	
	  ...
	  let x, q = P.solve0 p in
	    Format.eprintf "\nsolve --> %s = %s@?" (to_string (P.indet x)) (to_string p);
	    ()
	with
	    _ -> ()
  end 
