...
(* $Id: hasht.ml,v 1.1 2001/01/30 21:20:51 dave_sc Exp $ *)
...
let rec bucketlist_length = function
  | Empty -> 0
  | Cons (_,_,bl) -> succ (bucketlist_length bl)
...
let stat h =
  ...
  Printf.printf 
    "%6d val, used = %6d / %6d (%2d%%), max length = %6d / %6d\n" 
    !t !ne size p !m h.max_len
...
module Make(H: HashedType): (S with type key = H.t) =
  ...
  let rec exists = function
    | Empty -> false
    | Cons (y,_,l) -> H.equal y key || exists l
  in
  for j = 0 to Array.length h.data - 1 do
    if j <> i && exists h.data.(j) then begin H.pp key; failwith "bang" end
  done;
  ***)
