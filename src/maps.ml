...
module Make(Key: OrderedType)(Val: EqType) = struct
  ...
  let is_functional m = 
    for_all (fun x u -> for_all (fun y v ->
      if Key.compare x y == 0 then Val.equal u v else true) m) m
  ...
  let to_balanced m = 
    fold Balanced.add m Balanced.empty
  ...
  let bndng x m = 
    ...
    let b = Graph.choose_if keyx m in 
      ...
      assert(try Val.equal (snd(b)) (Balanced.find x (to_balanced m)) with _ -> false);
      b
  ...
  let find x m = 
    ...
    let v = snd(bndng x m) in
      assert(try Val.equal v (Balanced.find x (to_balanced m)) with _ -> false);
      v  
  ...
  let set x v m = 
    assert(well_formed m);
    assert(b := to_balanced m; true);
    try
      let xu = bndng x m in
	Graph.remove xu m;
	Graph.add (x, v) m
    with
	Not_found -> Graph.add (x, v) m;
    assert(well_formed m);
    assert(mem x m);
    assert(try Val.equal (find x m) v with _ -> false) (* ;
    assert(Balanced.equal Val.equal (to_balanced m) (Balanced.add x v !b)) *)
    
  let remove x m = 
    assert(well_formed m);
    assert(b := to_balanced m; true);
    try Graph.remove (bndng x m) m with Not_found -> ();
    assert(well_formed m);
    assert(not(mem x m)) (* ;
    assert(Balanced.equal Val.equal (to_balanced m) (Balanced.remove x !b)) *)
  ...
  let destruct p m = 
    ...
    let m' = copy m in  
      ...
      assert(not(mem x m));
(*
      assert(Balanced.equal Val.equal (to_balanced m) (Balanced.add x v (to_balanced m'))); *)
      x, v, m'
  ...
  let equal m1 m2 = 
    ...
    Graph.subset m1 m2 && Graph.subset m2 m1  (* not really. *)
end
