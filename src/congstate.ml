...
let apply s =
  let rec loop x =
    ...
    if eq_term x y then x else loop y
  in
  loop
...
let update s x y =
  assert (not (eq_term x y)); (* && not(is_const x)); *)
  let y' = find s x in
  if not(eq_term y y') then
    begin
...
let pp_find s = Pp.tmap Pp.term s.find
let pp_use s = Pp.tmap Pp.tset s.use
let pp_universe s = Pp.tmap Pp.term s.find
