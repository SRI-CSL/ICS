...
module Product(Ordered1: ORDERED)(Ordered2: ORDERED) = struct
  ...
  let fill p a b = p.lhs <- a; p.rhs <- b
  let lhs p = p.lhs
  let rhs p = p.rhs
  let equal p q =
    Ordered1.equal p.lhs q.lhs &&
    Ordered2.equal p.rhs q.rhs
  let compare p q = 
    let c1 = Ordered1.compare p.lhs q.lhs in
      if c1 <> 0 then c1 else Ordered2.compare p.rhs q.rhs
  let hash p = 
    (Ordered1.hash p.lhs + Ordered2.hash p.rhs) land 0x3FFFFFFF
  let pp fmt p = 
    Format.fprintf fmt "@[<";
...
module Triple(Ordered1: ORDERED)(Ordered2: ORDERED)(Ordered3: ORDERED) = struct
  type elt1 = Ordered1.t
  type elt2 = Ordered2.t
  type elt3 = Ordered3.t
  type t = {mutable arg1 : elt1; mutable arg2 : elt2; mutable arg3 : elt3; mutable hash : int}
  let make a b c = {arg1 = a; arg2 = b; arg3 = c; hash = -1}
  let dummy () = 
    let arb1 = Obj.magic None and arb2 = Obj.magic None and arb3 = Obj.magic None in
      make arb1 arb2 arb3
  let fill p a b c = p.arg1 <- a; p.arg2 <- b; p.arg3 <- c; p.hash <- -1
  let arg1 p = p.arg1
  let arg2 p = p.arg2
  let arg3 p = p.arg3
  let equal p q =
  ...
  let hash p = 
    if p.hash >= 0 then p.hash else
      let h = (Ordered1.hash p.arg1 + Ordered2.hash p.arg2 + Ordered3.hash p.arg3) land 0x3FFFFFFF in
	p.hash <- h; h
  let pp fmt p = 
    Format.fprintf fmt "@[<";
