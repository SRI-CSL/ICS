 ...
 *)
 ...
 (** Polynomials.
   ...
   polynomials over a given field of coefficients {!Polynomial.COEFF}
   and indeterminates {!Polynomial.INDETERMINATE}. 

   @author Harald Ruess
 *)

(** {i Coefficients of polynomials.} 
  Signature of the {i coefficients} argument of the functor 
  {!Polynomial.Make}.  The structure [(t, add, minus, zero, mult, one, inv)] 
  is supposed to be a {i field}. *)
...
(** {i Indeterminates of polynomials.} 
  Signature of the {i indeterminates} argument of the 
  functor {!Polynomial.Make}. *)
...
module type P = sig
  ...
  (** Representation of polynomials of the form [c{0} + c{1}*x{1} + ... + c{n}*x{n}]
    with [c{i}] of nonzero coefficients of type [coeff] and [x{j}] of type [indet]. *)
  type t = private {
    constant : coeff;
    monomials : Map.t;
    mutable hash : int
  ...
  val equal : t -> t -> bool
    (** Two polynomials [c{0} + c{1}*x{1} + ... + c{n}*x{n}] and [d{0} + d{1}*y{1} + ... + d{,}*y{m}]
      are equal iff [n = m] and there is a permutation [pi] on [[1..n]] such that [c{i}] and [d{pi(i)}]
      are equal in the domain of coefficients and [x{i}] and [y{pi(i)}] are equal in the domain
      of indeterminates. *)
  ...
  val mem : indet -> t -> bool
    (** [mem x p] holds iff indeterminate [x] is in the domain of polynomial [p]. *)
  ...
  val multc : coeff -> t -> t
    (** [multc c p] returns a polynomial equivalent to the multiplication [c * p]
      of the constant polynomial [c] and the polynomial [p]. *)

  val addc : coeff -> t -> t
    (** [addc c p] returns a polynomial equivalent to the sum [c + p]
      of the constant polynomial [c] and the polynomial [p]. *)
  ...
  val add : t -> t -> t
    (** [add p q] returns a polynomial equivalent to the sum [p + q]
      of polynomials [p] and [q]. *)

  val sub : t -> t -> t
    (** [sub p q] returns a polynomial equivalent to the difference [p - q]
      of polynomials [p] and [q]. *)

  val mapc : (coeff -> coeff) -> t -> t
    (** [mapc f p] for a polynomial of the form [c{0} + c{1}*x{1} + ... + c{n}*x{n}]
      returns a polynomial equivalent to [f(c{0}) + f(c{1})*x{1} + ... + f(c{n})*x{n}]. *)
  ...
  val solve0 : t -> indet * t
    (** For a non-constant polynomial [p], [solve0 p] returns a {i solved form}
      [(x, q)] with [x] in the domain of [p] and [p = 0] iff [x = q]. Otherwise,
      if [p] is the [0] polynomial, then [solve0 p] raises [Valid],
      and if [p] is a constant polynomial different from [0], then [solve0 p]
      raises [Unsat]. *)
  ...
  val eval : (indet -> coeff) -> t -> coeff
    (** For a polynomial [p] of the form [c{0} + c{1}*x{1} + ... + c{n}*x{n}], and
      a valuation [rho] which is defined on the set of indeterminates of [p], 
      [eval rho p] returns the value  [c{0} + c{1}*rho(x{1}) + ... + c{n}*rho(x{n})]. *)

  val iter : (coeff -> unit) -> (indet -> coeff -> unit) -> t -> unit
    (** For a polynomial [p] of the form [c{0} + c{1}*x{1} + ... + c{n}*x{n}],
      [iter fc fm p] applies [fc c{0}] and [fm c{i} x{i}] for [i] in [[1..n]].
      The order in which these functions are applied is unspecified. *)

  val fold : (coeff -> 'a) -> (indet -> coeff -> 'a -> 'a) -> t -> 'a
    (** For a polynomial [p] of the form [c{0} + c{1}*x{1} + ... + c{n}*x{n}],
      [fold fc fm p] computes [fm x{pi(1)} c{pi{1)} (....(fm x{pi(n)} c{pi{n)} (fc c{0}))...)]
      with [pi] a fixed, but otherwise unspecified permutation on [[1..n]]. *)
  ...
  val for_all : (coeff -> bool) -> (indet -> coeff -> bool) -> t -> bool
    (** For a polynomial [p] of the form [c{0} + c{1}*x{1} + ... + c{n}*x{n}],
      [for_all fc fm p] holds iff  [fc c{0}] holds and [fm c{i} x{i}] 
      holds for all [i] in [[1..n]]. *)

  val exists : (coeff -> bool) -> (indet -> coeff -> bool) -> t -> bool
    (** For a polynomial [p] of the form [c{0} + c{1}*x{1} + ... + c{n}*x{n}],
      [exists fc fm p] holds iff  [fc c{0}] holds or [fm c{i} x{i}] holds
      for at least one [i] in [[1..n]]. *)
...
(** {!Polynomial.Make} constructs polynomials with
  the given coefficients and indeterminates. *)
