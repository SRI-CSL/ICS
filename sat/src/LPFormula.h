...
public:
	~LPFormula() {
		if (is_boolean_op()) {
			delete[] data.boolean_op.arguments;
		}
		else if (is_eq()) {
			ics_free_root(data.eq.atom);
			ics_free_root(data.eq.not_atom);
		}
	}
	...
	value get_atom() const { assert(is_eq()); return *(data.eq.atom); }
	value get_not_atom() const { assert(is_eq()); return *(data.eq.not_atom); }
...
class LPFormulaManager {
	...
	/* void expand_variable_array() { 
		MEM_TRACE(ctrace << "[memory] expanding variable array, num_variables = " << num_variables << endl;);
		LPTermId * old = variables;
		expand_array(variables, num_variables); 
		int delta = variables - old;
		for (unsigned int i = 1; i < next_formula_id; i++)
			if (formulas[i].is_exists())
				formulas[i].data.exists.variables += delta;
		MEM_TRACE(ctrace << "         new num_variables = " << num_variables << endl;);
	}
  ...
  */
	...
	/* 
	bool create_space_for_new_variables(unsigned int num_vars) {
		bool expanded = false;
		while (next_variable_pos + num_vars >= num_variables) {
...
public:
	...
	LPFormulaId create_atom(value a, value not_a);
